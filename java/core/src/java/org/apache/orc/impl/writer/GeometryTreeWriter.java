...
public class GeometryTreeWriter extends TreeWriterBase {
    ...
    public GeometryTreeWriter(TypeDescription schema,
    ...
    public void writeBatch(ColumnVector vector, int offset,
        ...
        if (vector.isRepeating) {
            if (vector.noNulls || !vector.isNull[0]) {
                ...
                indexStatistics.updateGeometry(vec.vector[0], vec.start[0], vec.length[0], length);
            ...
            for (int i = 0; i < length; ++i) {
                if (vec.noNulls || !vec.isNull[i + offset]) {
                    ...
                    indexStatistics.updateGeometry(bw);
                    if (createBloomFilter) {
                        if (bloomFilter != null) {
